...
export function getMagicLinkPayload(request: Request) {
  ...
  if (typeof magic !== "string") {
    throw invalidMagicLink("Magic search param does not exist");
  }
...
export function sendMagicLinkEmail(link: string, email: string) {
  ...
  const html = renderToStaticMarkup(
    <div>
      ...
      <p>
        Hey, there! Click the link below to finish logging in to React Router
        Recipes app.
      </p>
  ...
  return sendEmail({
    from: `React Router Recipes <postmaster@${process.env.MAILGUN_DOMAIN}>`,
    to: email,
